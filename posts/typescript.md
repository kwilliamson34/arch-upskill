...
Gradual typing: TypeScript uses the type any whenever it can’t tell what the type of an expression should be. Compared to Dynamic, calling `any` a type is an overstatement. It just turns off the type checker wherever it appears. `any` is contagious, too — if you initialize a variable with an expression of type `any`, the variable has type `any` too. To get an error when TypeScript produces an `any`, use "noImplicitAny": true, or "strict": true in tsconfig.json.
